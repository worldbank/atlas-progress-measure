...
# Input data
JMP_2025_WLD <- read_excel("input/JMP_2025_WLD.xlsx", 
                           sheet = "wat")

data_all <- JMP_2025_WLD %>%
  select(iso3, year, wat_sm_t, wat_imp_prem_t, wat_imp_qual_t, wat_imp_av_t) %>%
  rename(iso3c = iso3,
         date = year)

wdind <- c("wat_imp_prem_t", "wat_imp_qual_t", "wat_imp_av_t")
...
writexl::write_xlsx(dashboard_final, "intermediate/dashboard_output_water_components.xlsx")
